...
Now let's look at the keys, `description` is the longest, at 11 characters, while the average is much less. Even the longest string value would comfortable fit in 16 character vectors. We could look at 100 more examples and collect more accurate statistics but I feel confident that this is a fair assumption to draw and build on. Paying the cost to move 32 characters into a SIMD pack seems rather wasteful and unlikely to help. AVX does support 128bit vector instructions as well so it's worth a test just to get an idea of the difference it would make.
